...
# Script de instalación de Calibre en Raspberry Pi con Raspbian OS
# Este script instala Calibre y configura el servidor de libros electrónicos.
# Autor: [Tu Nombre]
# Fecha: [Fecha Actual]
...
log "Instalando dependencias necesarias (jq, curl)..."
apt-get update && apt-get install -y jq curl

# Verificar si jq se instaló correctamente
if ! command -v jq &> /dev/null; then
    log "Error: jq no se pudo instalar."
    exit 1
fi
...
# Instalar Calibre usando el contenido del instalador de Calibre
log "Descargando e instalando Calibre..."
install_calibre() {
    log "Iniciando instalación de Calibre..."
    ...
    search_for_python() {
        ...
        set -f
        for candidate_path in $PATH; do
            for candidate_python in $pythons; do
                if [ -x "$candidate_path/$candidate_python" ]; then
                    echo "$candidate_path/$candidate_python"
                    return
                fi
            done
        done
    ...
    log "Usando el intérprete de Python: $PYTHON"

    $PYTHON -c "import sys; script_launch=lambda:sys.exit('¡Fallo en la descarga del instalador!'); exec(sys.stdin.read()); script_launch()" "$@" <<'CALIBRE_LINUX_INSTALLER_HEREDOC'
# {{{
# HEREDOC_START
#!/usr/bin/env python
